...
$(document).ready(function(){
    ...
    $( "#target" ).submit(function( event ) {
        table.clear().draw();
       // alert("API/database.php?req=rep3&cid="+$("#class").val()+"&start="+st.format('YYYY-MM-DD')+"&end="+en.format('YYYY-MM-DD')+"&stud="+$("#stud").val());
       $.ajax({ url: "API/database.php?req=rep3&cid="+$("#class").val()+"&start="+st.format('YYYY-MM-DD')+"&end="+en.format('YYYY-MM-DD')+"&stud="+$("#stud").val(),
            context: document.body,
            success: function(data){
                data = JSON.parse(data); // Now it's an array
                console.log(data);
