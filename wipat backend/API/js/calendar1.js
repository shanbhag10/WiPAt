...
function getRealYear1(dateObj)
{
	return (dateObj.getYear() % 100) + (((dateObj.getYear() % 100) < 39) ? 2000 : 1900);
}

function getDaysPerMonth1(month, year)
{
	/* 
...
function createCalender1(year, month, day)
{
	 // current Date
	var curDate = new Date();
	var curDay = curDate.getDate();
	var curMonth = curDate.getMonth();
	var curYear = getRealYear1(curDate)
	...
	var yearFound = 0;
	for (var i=0; i<document.getElementById('selectYear1').options.length; i++)
	{
		if (document.getElementById('selectYear1').options[i].value == year)
		{
			document.getElementById('selectYear1').selectedIndex = i;
			yearFound = true;
			break;
		}
	}
	if (!yearFound)
	{
		document.getElementById('selectYear1').selectedIndex = 0;
		year = document.getElementById('selectYear1').options[0].value;		
	}
	document.getElementById('selectMonth1').selectedIndex = month;
	...
	var trs = []
	var ti = 0;
	while (d <= getDaysPerMonth1(month, year))
	{
		if (firstRow)
		{
		...
		td.onmouseover = new Function('highLiteDay1(this)');
		td.onmouseout = new Function('deHighLiteDay1(this)');
		if (targetEl)
			td.onclick = new Function('pickDate1('+year+', '+month+', '+d+')');
		else
...
function showCalender1(elPos, tgtEl)
{
	targetEl = false;
	...
	var positions = [0,0];
	var positions = getParentOffset1(elPos, positions);	
	...
	if (m == null)
	{
		trs = createCalender1(false, false, false);
		showCalenderBody1(trs);
	}
	else
	{
		if (m[1].substr(0, 1) == 0)
		...
		if (m[2].substr(0, 1) == 0)
			m[2] = m[2].substr(1, 1);
		m[2] = m[2] - 1;
		trs = createCalender1(m[3], m[2], m[1]);
		showCalenderBody1(trs);
	}

	hideSelect1(document.body, 1);
}
function showCalenderBody1(trs)
{
	var calTBody = document.getElementById('calender1');
...
function setYears1(sy, ey)
{
	...
	if (ey)
		endYear = curYear;
	document.getElementById('selectYear1').options.length = 0;
	var j = 0;
	for (y=ey; y>=sy; y--)
	{
		document.getElementById('selectYear1')[j++] = new Option(y, y);
	}
}
function hideSelect1(el, superTotal)
{
	if (superTotal >= 100)
	{
		return;
	...
	for (var c=0; c<totalChilds; c++)
	{
		...
		{
			if (thisTag.id != 'selectMonth1' && thisTag.id != 'selectYear1')
			{
				var calenderEl = document.getElementById('calenderTable1');
				var positions = [0,0];
				var positions = getParentOffset1(thisTag, positions);	// nieuw
		...
		{
			hideSelect1(thisTag, (superTotal+1));
		}
	}
}
...
function highLiteDay1(el)
{
	el.className = 'hlDay';
}
function deHighLiteDay1(el)
{
...
function pickDate1(year, month, day)
{
	month++;
...
function getParentOffset1(el, positions)
{
	positions[0] += el.offsetLeft;
	positions[1] += el.offsetTop;
	if (el.offsetParent)
		positions = getParentOffset1(el.offsetParent, positions);
	return positions;
}